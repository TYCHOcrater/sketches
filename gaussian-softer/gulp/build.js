...
var gulp = require('gulp'),
    ...
    sass = require('run-sequence'),
    uglify = require('gulp-uglify');

gulp.task('build', function(done) {
    runSeq('clean', ['buildsass', 'buildimg', 'buildjs'], 'buildhtml', 'shaders', 'models', 'textures', done);
});
...
gulp.task('models', () => {
    return gulp.src(global.paths.model)
        .pipe(gulp.dest(global.paths.dist + '/models'));
});

gulp.task('textures', () => {
    return gulp.src(global.paths.textures)
        .pipe(gulp.dest(global.paths.dist + '/textures'));
});